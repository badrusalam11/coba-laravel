...
<div class="card mb-3 text-center">
     @if ($posts[0]->image)
        <div style="max-height:350px; overflow:hidden">
            <img src="{{ asset('storage/'.$posts[0]->image) }}" class="card-img-top image-fluid" alt="{{ $posts[0]->category->name }}">   
        </div>
    @else
        ...
        <div class="col-md-4 mb-3">
            <div class="card">
                <a href="/posts?category={{ $post->category->slug }}" class="text-decoration-none">
                    {{-- {{ $post->category->name }} --}}
                    <div class="position-absolute text-white px-3 py-2" style="background-color:rgba(0,0,0,0.5)">{{ $post->category->name }}</div>
                </a>
                
                @if ($post->image)
                     <img src="{{ asset('storage/'.$post->image) }}" class="card-img-top image-fluid mt-3" alt="{{ $post->category->name }}">   
                @else
